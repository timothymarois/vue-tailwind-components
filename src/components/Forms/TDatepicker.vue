            ...
            <date-picker 
               ...
               :style="(width ? `width: ${width}` : '')">
                  <template #icon-calendar>
                     <t-icon value="calendar" class="text-gray-500" :class="{'text-gray-500': !errorState, 'red-700': errorState}" size="5" />
                  </template>
            </date-picker>
         </div>
      </div>
      ...
      fieldClasses() {
         if(!this.customStyle) {
            ...
            if (this.readonly) {
               ...
               if(this.errorState) {
                  defaultClasses = defaultClasses.concat(['border-red-400 focus:border-red-400 text-red-700 hover:text-red-800 placeholder:text-red-500']);
               } else {
